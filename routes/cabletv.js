...
const { sendUtilityTransactionEmail } = require('../services/EmailService'); // <-- added import
...
router.post('/purchase', async (req, res) => {
  ...
  try {
    ...
    const [user, kycValidation] = await Promise.all([
      ...
      validateTransactionLimit(userId, amount, 'NGNZ', 'CABLE_TV')
    ]);
    ...
    logger.info(`📋 Bill transaction ${uniqueOrderId}: initiated-api | cable_tv | ${amount} NGNZ | ✅ 2FA | ✅ PIN | ✅ KYC`);
      ...
      try {
        if (user && user.email) {
          const emailOptions = {
            ...
            account: customer_id,
            additionalNote: ebillsStatus === 'completed-api' ? 'Cable TV service activated / renewed' : 'Cable TV purchase is being processed',
          ...
          logger.info(`Utility email (Cable TV) sent to ${user.email} for request ${finalRequestId}`);
        } else {
          logger.warn(`No email on file for user ${userId} — skipping utility email`);
        }
      } catch (emailErr) {
        logger.error('Failed to send utility email for cable TV transaction', {
          userId,
    ...
    if (error.message && 
      ...
      return res.status(400).json({
        success: false,
        error: 'INSUFFICIENT_NGNZ_BALANCE',
        message: 'Insufficient NGNZ balance to complete the cable TV purchase',
        details: {
          currency: 'NGNZ',
