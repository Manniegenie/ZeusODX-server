...
/**
 * Validate betting funding request - NGNZ ONLY with 2FA
 */
...
/**
 * Validate NGNZ transaction limits for betting
 */
...
/**
 * Main betting funding endpoint with guaranteed unique order IDs - NGNZ ONLY with 2FA and KYC
 */
router.post('/fund', async (req, res) => {
  ...
  try {
    ...
    logger.info(`Betting funding request from user ${userId}:`, requestBody);
    ...
    const { customer_id, service_id, amount, payment_currency, twoFactorCode } = validation.sanitized;
    ...
    if (!validateTwoFactorAuth(user, twoFactorCode)) {
      logger.warn('Invalid 2FA attempt for betting funding', { userId });
    ...
    logger.info('2FA validation successful for betting funding', { 
    ...
    logger.info(`📋 Bill transaction ${uniqueOrderId}: initiated-api | betting | ${ngnzAmount} NGNZ | ✅ 2FA | ✅ KYC | ⚠️ Not Reserved`);
    logger.info(`Created betting transaction ${pendingTransaction._id} with unique OrderID: ${uniqueOrderId}, RequestID: ${uniqueRequestId}`);
    ...
    try {
      ...
      // 🔑 CRITICAL FIX: Use the fixed callEBillsBettingAPI function that uses VTUAuth properly
      ...
      if (apiError.code === 'ECONNABORTED' || apiError.message.includes('timeout')) {
        ...
        switch (errorData.code) {
          ...
          case 'invalid_service_id':
            statusCode = 400;
            errorMessage = 'Invalid betting service provider';
            errorCode = 'INVALID_SERVICE_ID';
            break;
          case 'below_minimum_amount':
            statusCode = 400;
            errorMessage = 'Amount below minimum (₦1,000)';
    ...
    // Step 10: eBills Betting API SUCCESS! Now reserve user balance
    try {
      logger.info(`eBills Betting API successful for ${uniqueRequestId}. Now reserving balance...`);
      ...
      logger.info(`📋 Bill transaction ${uniqueOrderId}: initiated-api | betting | ${ngnzAmount} NGNZ | ✅ 2FA | ✅ KYC | ✅ Reserved`);
      logger.info(`Successfully reserved ${ngnzAmount} ${currency} for user ${userId} after eBills Betting API success`);
    ...
    logger.info(`Betting transaction updated. Our OrderID: ${uniqueOrderId}, eBills OrderID: ${ebillsResponse.data.order_id}, Status: ${ebillsResponse.data.status}`);
    ...
    if (ebillsResponse.data.status === 'completed-api') {
      logger.info(`✅ Betting funding completed immediately for user ${userId}, order ${uniqueOrderId}`);
      ...
      logger.info(`⏳ Betting funding processing for user ${userId}, order ${uniqueOrderId}`);
      ...
      logger.warn(`⚠️ Unexpected status ${ebillsResponse.data.status} for betting order ${uniqueOrderId}`);
