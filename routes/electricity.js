...
router.post('/purchase', async (req, res) => {
  ...
  try {
    ...
    if (ebillsStatus === 'completed-api') {
      ...
      logger.warn(`⚠️ Unexpected status ${ebillsStatus} for electricity order ${uniqueOrderId}`);
    }
    ...
    if (ebillsResponse.data.status === 'completed-api') {
      logger.info(`✅ Electricity purchase completed immediately for user ${userId}, order ${ebillsResponse.data.order_id}`);
      ...
      logger.info(`⏳ Electricity purchase processing for user ${userId}, order ${ebillsResponse.data.order_id}`);
      ...
      logger.info(`💰 Electricity purchase refunded for user ${userId}, order ${ebillsResponse.data.order_id}`);
      ...
      logger.warn(`⚠️ Unexpected status ${ebillsResponse.data.status} for electricity order ${ebillsResponse.data.order_id}`);
