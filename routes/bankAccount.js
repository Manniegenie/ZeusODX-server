...
router.post('/add-bank', async (req, res) => {
  try {
    const userId = req.user && req.user.id;
    const body = req.body || {};
    const { accountNumber, bankName, accountName, bankCode } = body;
...
// DELETE: /api/user/delete-bank - Delete a bank account
// supports: body { accountId } OR body { accountNumber } OR query ?accountId= OR url param /delete-bank/:accountId
router.delete('/delete-bank/:accountId?', async (req, res) => {
  try {
    const userId = req.user && req.user.id;
    const body = req.body || {};
    let accountId = body.accountId || req.query.accountId || req.params.accountId;
    const accountNumber = body.accountNumber || req.query.accountNumber;
    ...
    // Fetch user first (we need it for accountNumber fallback and ownership)
    const user = await User.findById(userId);
    if (!user) {
    ...
    // If no accountId but an accountNumber is provided, attempt to resolve id
    if (!accountId && accountNumber) {
      accountId = user.getBankAccountIdByNumber(accountNumber);
      if (!accountId) {
        logger.warn('Bank account not found by accountNumber', { userId, accountNumber, source: 'delete-bank-account' });
    ...
    if (!accountId) {
      ...
      return res.status(400).json({ message: 'Account ID or Account Number is required' });
    }

    // Locate the bank account subdocument
    const bankAccount = user.bankAccounts.id(accountId);
    if (!bankAccount) {
      logger.warn('Bank account not found', { userId, accountId, source: 'delete-bank-account' });
      return res.status(404).json({ message: 'Bank account not found' });
    }
    ...
    // Remove and save
    try {
      await user.removeBankAccount(accountId); // uses model method defined on User schema
      logger.info('Bank account deleted successfully', { userId, accountId, deletedAccount: deletedAccountInfo, source: 'delete-bank-account' });
      ...
      return res.json({
        ...
        deletedAccount: {
          ...
          bankCode: deletedAccountInfo.bankCode,
          accountNumber: deletedAccountInfo.accountNumber
        }
      });
    } catch (deleteError) {
      logger.error('Error during bank account deletion', { error: deleteError.message, userId, accountId, source: 'delete-bank-account' });
